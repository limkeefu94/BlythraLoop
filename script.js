...
function showPolicyModal(config, type) {
    ...
    // ✅ 全部已读 / 清除全部
    const markAllBtn = document.getElementById('markAllNotiRead');
    ...
    // 🔥 [Fix] 删除单条通知（之前 UI 有按钮但没有实现）
    window.deleteNotification = (id) => {
        const currentUser2 = currentMode === 'owner' ? 'admin' : loggedInCustomerName;
        let allData = JSON.parse(localStorage.getItem('gembrow_data') || '[]');
        const target = allData.find(item => item.type === 'notification' && String(item.id) === String(id));
        if (!target) return;

        // 系统通知：不全局删除，只对当前用户隐藏（加入已读列表）
        if (target.targetUser === 'all') {
            let readIds = JSON.parse(localStorage.getItem(`read_sys_notis_${currentUser2}`) || '[]');
            if (!readIds.some(rid => String(rid) === String(id))) {
                readIds.push(id);
                localStorage.setItem(`read_sys_notis_${currentUser2}`, JSON.stringify(readIds));
            }
        } else {
            allData = allData.filter(item => !(item.type === 'notification' && String(item.id) === String(id)));
            localStorage.setItem('gembrow_data', JSON.stringify(allData));
        }
    ...
    window.markAllNotificationsRead = () => {
        ...
        allData.forEach(item => {
            ...
            const isForMe = (item.targetUser === currentUser2) || (!item.targetUser && currentUser2 === 'admin');
        ...
        // 系统通知：写入 read list
        const allSystemIds = allData
            .filter(n => n.type === 'notification' && n.targetUser === 'all')
            .map(n => n.id);
        let readIds = JSON.parse(localStorage.getItem(`read_sys_notis_${currentUser2}`) || '[]');
        const merged = [...new Set([...readIds.map(String), ...allSystemIds.map(String)])];
        localStorage.setItem(`read_sys_notis_${currentUser2}`, JSON.stringify(merged));

        if (changed) localStorage.setItem('gembrow_data', JSON.stringify(allData));
    ...
    window.clearAllNotifications = () => {
        ...
        allData = allData.filter(item => {
            ...
            const isForMe = (item.targetUser === currentUser2) || (!item.targetUser && currentUser2 === 'admin');
            // 系统通知不删（全局）
            if (item.targetUser === 'all') return true;
            return !isForMe;
        });

        if (allData.length !== beforeLen) {
            localStorage.setItem('gembrow_data', JSON.stringify(allData));
        }
        ...
        <div class="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-scale-in h-[80vh] flex flex-col">
            <div class="p-4 bg-gray-50 border-b flex justify-between items-center shrink-0">
                ...
                <div class="flex gap-2 items-center">
                    <button id="markAllNotiRead" class="text-xs px-3 py-1 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold">全部已读</button>
                    <button id="clearAllNoti" class="text-xs px-3 py-1 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-700 font-bold">清除全部</button>
    ...
    modal.innerHTML = `
        <div class="bg-white w-full max-w-sm rounded-2xl shadow-2xl overflow-hidden animate-scale-in relative">
            ...
            <div class="p-6 space-y-4">
                ...
                <div class="flex justify-between border-b border-gray-100 pb-3">
                    <span class="text-gray-500 text-sm">预约日期</span>
                    <span class="font-bold text-gray-800">${booking.appointmentDate || '-'}</span>
                </div>
                <div class="flex justify-between border-b border-gray-100 pb-3 mt-2">
...
// 辅助：创建通知 (完全独立版)
function createNotification(targetUser, title, msg, opts = {}) {
    const noti = {
        type: 'notification',
        category: 'alert',
        subtype: 'automation',
